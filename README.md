...
PDF:
```javascript
javascript:!function(){if("undefined"==typeof html2pdf){var e=document.createElement("script");e.src="https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js",e.onload=t,document.head.appendChild(e)}else t();function t(){let e=Array.from(document.getElementsByClassName("font-claude-message"));if(0===e.length){alert("No Claude responses found in the conversation.");return}let t=document.createElement("div");t.style.padding="20px",t.style.backgroundColor="white",t.style.fontFamily="Arial, sans-serif",t.style.fontSize="14px",t.style.color="#2D3748",e.forEach((e,l)=>{var s;let a=document.createElement("div");a.style.marginBottom="30px",a.style.padding="20px",a.style.borderBottom="1px solid #E2E8F0",a.style.breakInside="avoid-page",a.style.pageBreakInside="avoid";let o=document.createElement("div");o.style.fontSize="16px",o.style.fontWeight="bold",o.style.color="#4A5568",o.style.marginBottom="15px",o.textContent=%60Response #${l+1}%60;let i=e.cloneNode(!0),n=((s=i).querySelectorAll("ol, ul").forEach(e=>{e.style.paddingLeft="30px",e.style.marginLeft="15px",e.style.listStylePosition="outside","OL"===e.tagName?e.style.listStyleType="decimal":e.style.listStyleType="disc",e.querySelectorAll("li").forEach(e=>{e.style.marginBottom="10px",e.style.paddingLeft="10px",e.style.breakInside="avoid",e.style.pageBreakInside="avoid"})}),s.querySelectorAll("p").forEach(e=>{e.style.marginBottom="15px",e.style.lineHeight="1.5",e.style.breakInside="avoid",e.style.pageBreakInside="avoid"}),s);a.appendChild(o),a.appendChild(n),t.appendChild(a)});let l={margin:[20,20,20,20],filename:"claude-responses.pdf",image:{type:"jpeg",quality:.98},html2canvas:{scale:2,useCORS:!0,logging:!1,scrollY:-window.scrollY,windowHeight:document.documentElement.offsetHeight},jsPDF:{unit:"mm",format:"a4",orientation:"portrait"},pagebreak:{mode:["avoid-all","css","legacy"]}};html2pdf().from(t).set(l).save().then(()=>console.log("PDF generated successfully!")).catch(e=>{console.error("Error generating PDF:",e),alert("Error generating PDF. Check console for details.")})}}();
```
