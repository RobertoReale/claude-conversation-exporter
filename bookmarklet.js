javascript:(function(){
    ...
    const SELECTORS = {
        ...
        CODE_BLOCKS: "pre",
        INLINE_CODE: "code",
        MATH_ELEMENTS: ".katex, .katex-display, .math, .math-display" // Aggiunto selettore per elementi matematici
    };
...
async function initExport(format) {
    ...
    const mathElements = document.querySelectorAll(SELECTORS.MATH_ELEMENTS);
    ...
    // Estrai stili per elementi matematici KaTeX
    if (mathElements.length > 0) {
        const computedStyle = window.getComputedStyle(mathElements[0]);
        mathStyle = {
            fontFamily: computedStyle.fontFamily,
            fontSize: computedStyle.fontSize,
            lineHeight: computedStyle.lineHeight,
...
async function generatePDF(container, filename) {
    ...
    // Apply post-processing on the clone
    processCodeFormatting(clone);
    processKaTeXFormatting(clone); // Nuova funzione per processare formule KaTeX
    processBoldTextFormatting(clone); // Nuova funzione per processare il testo in grassetto
...
// Nuova funzione per processare elementi KaTeX
function processKaTeXFormatting(container) {
    // Trova tutti gli elementi KaTeX
    const katexElements = container.querySelectorAll('.katex, .katex-display, .katex-html');
    
    katexElements.forEach(element => {
        // Applica stili specifici per correggere il rendering KaTeX
        ...
        // Correggi spaziatura numeratore/denominatore
...
// Nuova funzione per processare il testo in grassetto
function processBoldTextFormatting(container) {
    ...
    boldElements.forEach(element => {
        ...
        // Aggiungi altri stili per garantire il corretto allineamento
